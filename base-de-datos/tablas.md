...
Un registro por evento por usuario.
...
| `canal_push`          | BOOLEAN                                                                                                                                                            |
| `notif_evento` (enum) | recurrente_dia_antes, corte_tarjeta_dia_antes, meta_completada, meta_progreso, presupuesto_alerta, presupuesto_excedido, deuda_vencida, cuenta_cobrar_recordatorio |
...
| `nombre`     | VARCHAR                                                     |
| `cupo_total` | DECIMAL                                                     |
| `Cálculo`    | disponible = suma de saldos de todas las tarjetas del grupo |
...
| Columna      | Notas                                                   |
| ------------ | ------------------------------------------------------- |
| `es_sistema` | TRUE para las categorías predeterminadas (no borrables) |
| `parent_id`  | Autorreferencial (subcategorías)                        |

### `etiquetas` + `transacciones_etiquetas` (tabla pivote)

| Entidad                   | Notas                                        |
| ------------------------- | -------------------------------------------- |
| `etiquetas`               | Tags libres creados por el usuario           |
| `transacciones_etiquetas` | Relación M:N entre transacciones y etiquetas |
...
| Columna             | Notas                                  |
| ------------------- | -------------------------------------- |
| `periodo`           | mensual, semanal, anual, personalizado |
| `alerta_porcentaje` | Default 80                             |
| `activo`            | BOOLEAN                                |
...
| Columna            | Notas                                    |
| ------------------ | ---------------------------------------- |
| `tipo`             | cuenta_virtual, cuenta_especifica, libre |
| `monto_actual`     | Actualizado en cada aporte               |
| `completada`       | BOOLEAN                                  |
...
| Columna                   | Notas              |
| ------------------------- | ------------------ |
| `metodo_valoracion`       | automatico, manual |
...
| Columna           | Notas                                 |
| ----------------- | ------------------------------------- |
| `raw_json`        | JSONB — JSON completo para reprocesar |
| `historico_score` | JSONB — últimos 3 períodos            |
| `archivo_path`    | ruta del JSON original guardado       |

### `cripto_precios_historico`

| Uso       | Notas                                                                                     |
| --------- | ----------------------------------------------------------------------------------------- |
| `General` | Registro por cada actualización de precio para graficar evolución. Crecerá con el tiempo. |
...
| Columna               | Notas                                   |
| --------------------- | --------------------------------------- |
| `estado`              | pendiente, parcial, cobrado, incobrable |
| `ultimo_recordatorio` | DATE (para evitar spam)                 |

### `cuentas_por_cobrar_abonos`

| Uso       | Notas                                       |
| --------- | ------------------------------------------- |
| `General` | Historial de pagos parciales de cada deuda. |

### `deudas_informales` + `deudas_informales_abonos`

| Uso       | Notas                                                                |
| --------- | -------------------------------------------------------------------- |
| `General` | Espejo de cuentas_por_cobrar pero desde la perspectiva de "yo debo". |
